...
def parse_instruction(line):
    """Parses a single RISC-V instruction line and encodes it."""
    parts = line.split()
    if len(parts) < 2:
        print(f"Error parsing instruction '{line}': Not enough arguments.")
        return None
    ...
    try:
        ...
        elif instr in ['addi', 'andi', 'ori', 'slti', 'sltiu', 'slli', 'srli', 'srai']:
            if len(parts) != 4:
                raise ValueError(f"Invalid I-type format: {line}")
            rd, rs1, imm = parts[1], parts[2], parts[3]
            return encode_i_type(instr, rd, rs1, imm)

        elif instr in ['lw', 'sw']:
            if len(parts) != 3:
                raise ValueError(f"Invalid format for memory instruction: {line}")
            rd_rs, offset_rs1 = parts[1], parts[2]
            match = re.match(r'(-?\d+)\((x\d+)\)', offset_rs1)
            if not match:
                raise ValueError(f"Invalid memory access format: {line}")
            offset, rs1 = match.groups()
            if instr == 'lw':
                return encode_i_type(instr, rd_rs, rs1, offset)
            else:
                return encode_s_type(instr, rs1, rd_rs, offset)

        elif instr in ['beq', 'bne', 'blt', 'bge', 'bltu', 'bgeu']:
            if len(parts) != 4:
                raise ValueError(f"Invalid B-type format: {line}")
            rs1, rs2, imm = parts[1], parts[2], parts[3]
            return encode_b_type(instr, rs1, rs2, imm)

        elif instr == 'jal':
            if len(parts) != 3:
                raise ValueError(f"Invalid J-type format: {line}")
            rd, imm = parts[1], parts[2]
            return encode_j_type(instr, rd, imm)

        elif instr in ['lui', 'auipc']:
            if len(parts) != 3:
                raise ValueError(f"Invalid U-type format: {line}")
            rd, imm = parts[1], parts[2]
            return encode_u_type(instr, rd, imm)

        else:
            print(f"Error parsing instruction '{line}': Unknown instruction '{instr}'.")
            return None

    except Exception as e:
        print(f"Error parsing instruction '{line}': {e}")
        return None
